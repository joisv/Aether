<div x-data="">
    <x-tables.table name="Series" count="{{ $products->count() }} Products">
        ...
        <x-slot name="addBtn">
            <x-tables.button type="button" x-data=""
        ...
        <x-slot name="thead">
            ...
            <x-tables.th>Visibility</x-tables.th>
            <x-tables.th>Title</x-tables.th>
            <x-tables.th>Stock</x-tables.th>
                ...
                <tr>
                    <x-tables.td>
                        <input id="default-{{ $index }}" type="checkbox"
                            ...
                            wire:model.live="mySelected" value="{{ $product->id }}">
                    </x-tables.td>
                    @if ($product->visible)
                        <x-tables.td>
                            <x-icon name="c-eye" />
                    ...
                    <x-tables.td>{{ $product->created_at->format('F j, Y') }}</x-tables.td>
                    <x-tables.td>{{ $product->updated_at->format('F j, Y') }}</x-tables.td>
                    ...
                    <x-tables.td>
                        <div class="flex space-x-1 items-center">
                            ...
                            <button type="button"
                                class="flex items-center justify-center w-1/2 px-5 py-2 text-sm disabled:text-gray-700 transition-colors duration-200 disabled:bg-white border rounded-lg gap-x-2 sm:w-auto dark:hover:bg-gray-800 dark:bg-gray-900 hover:bg-gray-100 dark:text-gray-200 dark:border-gray-700 bg-red-500 text-white"
                                wire:click="destroyAlert({{ $product->id }})"
